// Deploys contracts/quotekeeper.py to GenLayer testnet (Bradbury by
...
async function main() {
  ...
  const contractFile = process.argv[2] ?? "quotekeeper.py";
  const code = readFileSync(join(CONTRACTS_DIR, contractFile), "utf-8");
